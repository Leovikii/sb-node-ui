...
名称与备注只允许在编辑态通过 Mantine 输入组件修改；预览态以只读文本展示当前草稿的名称与备注，并隐藏保存操作。模式切换与底部操作区位于可滚动业务内容之外，长表单和代码预览统一使用 Mantine `ScrollArea` 独立滚动，避免取消和保存按钮随内容离开视口。移动端继续使用全屏 Modal，不实现自定义 sticky、滚动锁或 overlay。
