...
回滚基线：`3.0.0`
当前候选：`3.1.0-beta.1`
状态：React/Mantine Beta 已部署生产并完成初步功能验证；FE-3157 已在本地完成，等待用户后续部署。
...
- 生产反馈：用户已将 `3.1.0-beta.1` 部署生产并完成初步功能验证，未发现功能异常；本任务变更尚未再次部署。
...
- 遗留风险：当前生产环境仍是 FE-3157 之前的 Beta 构建；FE-3151 的正式版总 JS 预算仍未完成。
- 下一任务：用户部署本次 UI 修复后进行生产只读冒烟；随后继续 FE-3151 体积收束。
